"""Hyperparameter sweep for CMA-ES on Tomato.

One-at-a-time sweeps over population size, initial step size,
initialization mode, and repair mode.
Each config run with 3 seeds. Saves results to results/tuning_cmaes.json.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out", type=Path, default=Path("results") / "tuning_cmaes.json")
    return p.parse_args()
...
def main() -> None:
    ...
    best_path = args.out.parent / "best_cmaes.json"
    payload = {
